...
- ソースコードはリポジトリへ push し、インストーラーや各種アーカイブはソースツリーへ入れずに GitHub release 資産としてアップロードします。
- 正式版ごとに新しい tag と新しい release を作成し、旧 release は明示指示がない限り削除や上書きをしません。
- ローカル実行成果物、`node_modules`、`dist`、一時パッケージングフォルダー、ログ、その他の build 副産物は同期しません。
- `AgentLogic`、ローカル専用ルール文書、非公開の計画メモ、ランタイムキャッシュ、その他の協作用副産物も同期しません。
